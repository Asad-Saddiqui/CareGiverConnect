<?php
    ...
    $id=$_SESSION['CaretakerID'];
    // Default search query to retrieve all caregivers
    $search = "SELECT * FROM meetings LEFT JOIN user ON user.id = meetings.gid WHERE role='Caregiver' and tid=$id;";
    ...
    if (isset($_POST['search'])) {
        ...
        $search = "SELECT * FROM meetings LEFT JOIN user ON user.id = meetings.gid WHERE role='Caregiver' AND tid=$id AND (name LIKE '%$search_query%' OR subject LIKE '%$search_query%')";
    }
?>
                             ...
                             while ($rows_= mysqli_fetch_assoc($result)) {
                              ...
                              $count=$count + 1;
                                ...
                                <tr>
                                    ...
                                    <td><?php if((int)$rows_['accept']===1){
                                        echo "accepted";
                                    }else{
                                        echo "Pending";
                                    }?></td>
                                    <td><?php if((int)$rows_['status']===1){
                                        echo "Done";
                                    }else{
                                        echo "Pending";
                                    }?></td>
                                    <td>
                                        <a href="delete-request.php?mid=<?php echo $rows_['mid']; ?>">
                                            <button class="btncls">delete</button>
                                </a>
                                </td>
                                </tr>
                                <?php }  ?>
